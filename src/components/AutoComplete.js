...
const AutoComplete = (props) => {
  ...
  const fetchData = (query) => {
    let fetchURL;
    const { url, header } = getAutoSuggestURLDetails({
      domain,
      query,
      showSubTasks,
    });
      ...
      fetch(fetchURL, header)
        ...
        .then((data) => {
          ...
          if (process.env.NODE_ENV === 'development') {
            ...
            const dataList =
              data?.sections?.[0]?.issues.length > 0 ? data.sections?.[0].issues : [];
      ...
      suggestionsListComponent = (
        <ul className="suggestions">
          {filteredSuggestions.map((suggestion, index) => {
            ...
            return (
              <li className={className} key={suggestion.id} onClick={() => onClick(suggestion.key)}>
